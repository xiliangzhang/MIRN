...
from deepctr.layers.utils import reduce_max, reduce_mean, reduce_sum, concat_func, softmax
...
class SampledSoftmaxLayer(Layer):
    ...
    def call(self, inputs_with_label_idx, training=None, **kwargs):
        ...
        loss = tf.nn.sampled_softmax_loss(weights=embeddings,  # self.item_embedding.
                                          ...
                                          labels=label_idx,
                                          inputs=inputs,
                                          num_sampled=self.num_sampled, # 采样个数
                                          num_classes=self.size,  # self.target_song_size 分类类别
                                          )
